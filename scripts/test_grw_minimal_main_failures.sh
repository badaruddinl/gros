...
expect_failure() {
    ...
    grep -F 'unsupported source' "$TMP_DIR/$name.err" > /dev/null || fail "$name: missing rejection reason"
    pass "$name"
}
...
expect_failure "comment" $'target "gros.x86.bios.real16.stage2.v0"\n// comments are not part of this subset\nfn main() -> void { return; }'
