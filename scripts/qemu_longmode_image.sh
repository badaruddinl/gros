...
grep -F 'LM64IDTPMEMF1HEAPT1T2FSOKEX06' "$LOG" > /dev/null || { echo 'error: filesystem/exception marker missing' >&2; exit 1; }
echo 'long mode: qemu initialized physical memory, heap, scheduler, boot filesystem, and exception handler'
