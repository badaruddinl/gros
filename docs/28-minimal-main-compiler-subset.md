...
The local Bash compiler is `scripts/grw_minimal_main.sh`. It normalizes LF and
CRLF and accepts `//` line comments, then emits deterministic
...
- `scripts/test_grw_minimal_main_failures.sh` accepts comments/CRLF and rejects
  unsupported target, body, extra function, duplicate target, and block comment;
...
This does not implement a general parser, block comments, declarations, expressions,
